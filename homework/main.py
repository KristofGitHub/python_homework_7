# ЗАДАЧА 1. Винни-Пух попросил Вас посмотреть, есть ли в его стихах ритм. Поскольку разобраться в его кричалках не 
...
def VowelsCount(word):
    vowelsCount = 0
    vowelsRus = "аеёиоуыэюяАЕЁИОУЫЭЮЯ"
    vowelSeparateArray = [char for char in vowelsRus]
    wordSeparateArray = [char for char in word]
    for i in range (0, len(wordSeparateArray)):
        for j in range (0, len(vowelSeparateArray)):
            if (wordSeparateArray[i] == vowelSeparateArray[j]): vowelsCount += 1
    return vowelsCount

vinniePoem = str(input("Введите поэму Винни: "))
res = list(map(VowelsCount, vinniePoem.split()))
ok = True
for i in range (0, len(res)):
    for j in range (i, len(res)):
        if (res[i] != res[j]): ok = False
if (ok == True): print("Парам пам-пам (ритм на месте)")
else: print("Пам парам (ритма нет)")
#################################################################################################################################

# ЗАДАЧА 2. Напишите функцию print_operation_table(operation, num_rows=6, num_columns=6), которая принимает в качестве 
